(function ($) {
    ...
    const galleryItems = Array.from(document.querySelectorAll('.landing-gallery__item'));
    const galleryPagination = document.querySelector('.landing-gallery__pagination');
    const galleryTriggers = Array.from(document.querySelectorAll('.landing-gallery__trigger'));
    ...
    const updateGalleryModal = () => {
        ...
        const currentImage = galleryTriggers[activeGalleryIndex]?.querySelector('img');
        if (!currentImage) {
            return;
        }
    ...
    if (galleryModalElement && galleryTriggers.length) {
        ...
        galleryNavButtons.forEach((button) => {
            button.addEventListener('click', () => {
                ...
                if (button.dataset.galleryNav === 'next' && activeGalleryIndex < galleryTriggers.length - 1) {
                    activeGalleryIndex += 1;
                }
    ...
    const aboutRevealItems = Array.from(document.querySelectorAll('.about-page [data-reveal]'));
    const aboutParallaxItems = Array.from(document.querySelectorAll('.about-page [data-parallax]'));
    ...
    if (aboutPage && aboutParallaxItems.length && !prefersReducedMotion) {
        ...
        const updateAboutParallax = () => {
            aboutParallaxItems.forEach((item) => {
                ...
                const offset = Math.max(Math.min(distance * -strength, strength), -strength);
