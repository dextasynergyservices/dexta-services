  ...
  return (
    ...
    >
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div>
          <Label className="mb-1.5 block text-xs text-[#888]">Metric label</Label>
          <Input
      ...
      <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_180px]">
        ...
        <div>
          <Label className="mb-1.5 block text-xs text-[#888]">Metric value</Label>
          <Input
            className="border-[#2a2a2a] bg-[#0d0d0d] text-white"
      ...
      <label className="flex items-center justify-between rounded-xl border border-[#222] bg-[#0d0d0d] px-4 py-3">
        <div>
          ...
          <p className="text-xs text-[#666]">Hide this expertise card without deleting it.</p>
        </div>
        <Switch
      ...
      <div className="flex justify-end gap-3">
        ...
        <Button type="submit" className="bg-cyan-500 text-[#03131d] hover:bg-cyan-400">
          {isSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : "Save Expertise"}
        </Button>
      </div>
    </form>
  );
...
export function ExpertiseManager({ items }: { items: AboutExpertiseItemRow[] }) {
  const router = useRouter();
  const [isCreateOpen, setIsCreateOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<AboutExpertiseItemRow | null>(null);
  const [deletingItem, setDeletingItem] = useState<AboutExpertiseItemRow | null>(null);
    ...
    <>
          ...
          {items.map((item, index) => (
            <article key={item.id} className="rounded-2xl border border-[#222] bg-[#111] p-5">
              ...
              <div className="mt-4 rounded-xl border border-[#222] bg-[#0d0d0d] px-4 py-3">
                <p className="text-xs uppercase tracking-[0.2em] text-[#555]">{item.metricLabel}</p>
                <p className="mt-1 text-sm font-medium text-white">{item.metricValue}</p>
              </div>
              <div className="mt-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <CardActionButton onClick={() => setEditingItem(item)} label="Edit expertise card">
                    <Pencil className="h-4 w-4" />
                  </CardActionButton>
                  <CardActionButton onClick={() => setDeletingItem(item)} label="Delete expertise card">
      ...
      <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
        <DialogContent className="border-[#222] bg-[#111] text-white sm:max-w-2xl">
          ...
          <ExpertiseForm onSubmit={handleCreate} onCancel={() => setIsCreateOpen(false)} />
        </DialogContent>
      </Dialog>
      ...
      <AlertDialog open={!!deletingItem} onOpenChange={(open) => !open && setDeletingItem(null)}>
        <AlertDialogContent className="border-[#222] bg-[#111]">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-white">Delete expertise card?</AlertDialogTitle>
