    ...
    >
      <div>
        ...
        <Select
          value={duration}
          onValueChange={(v) =>
            setValue("duration", v as PlanBillingOptionInput["duration"], {
              shouldDirty: true,
              shouldValidate: true,
            })
          }
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          <Label htmlFor="opt-ngn" className="mb-1.5 block text-xs text-[#888]">
            Price NGN <span className="text-[#555]">(optional)</span>
          </Label>
          <Input
            id="opt-ngn"
            type="number"
          ...
          {errors.priceNGN ? (
            <p className="mt-1 text-xs text-red-400">
              {errors.priceNGN.message}
            </p>
          ) : null}
        </div>
        <div>
          <Label htmlFor="opt-usd" className="mb-1.5 block text-xs text-[#888]">
            Price USD <span className="text-[#555]">(optional)</span>
          </Label>
          <Input
            id="opt-usd"
            type="number"
      ...
      <div>
        <Label htmlFor="opt-label" className="mb-1.5 block text-xs text-[#888]">
          Display label{" "}
          <span className="text-[#555]">
            (optional — overrides duration name)
          </span>
        </Label>
        <Input
          id="opt-label"
      ...
      <div className="flex items-center gap-3 rounded-lg border border-[#222] bg-[#0d0d0d] px-4 py-3">
        <Switch
          ...
          checked={isDefault}
          onCheckedChange={(c) =>
            setValue("isDefault", c, { shouldDirty: true })
          }
        />
        <div>
          <Label htmlFor="opt-default" className="text-xs text-[#888]">
            Set as default selection
          </Label>
          <p className="text-[11px] text-[#555]">
            This option will be pre-selected when the card loads.
          </p>
        </div>
      </div>
