...
export function AudienceSettingsForm({ audience, onSubmit, onCancel }: Props) {
  ...
  } = useForm<FormValues, unknown, AudienceInput>({
    resolver: zodResolver(audienceSchema),
    defaultValues: toFormValues(audience),
  });

  useEffect(() => {
    reset(toFormValues(audience));
  }, [audience, reset]);
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div>
          ...
          <Select
            value={color}
            onValueChange={(v) =>
              setValue("color", v as AudienceInput["color"], {
                shouldDirty: true,
                shouldValidate: true,
              })
            }
          ...
          {errors.color ? (
            <p className="mt-1 text-xs text-red-400">{errors.color.message}</p>
          ) : null}
        </div>
      </div>
      ...
      <div>
        ...
        <Input
          id="aud-empty-title"
          placeholder="e.g. School offers are being refreshed."
          className="border-[#2a2a2a] bg-[#0d0d0d] text-white placeholder-[#444] focus-visible:border-cyan-500/50 focus-visible:ring-cyan-500/20"
          {...register("emptyTitle")}
        />
        {errors.emptyTitle ? (
      ...
      <div>
        ...
        <Textarea
          id="aud-empty-body"
          rows={3}
          placeholder="Support text when this audience has no visible groups."
      ...
      <div className="flex items-center gap-3 rounded-lg border border-[#222] bg-[#0d0d0d] px-4 py-3">
        ...
        />
        <Label htmlFor="aud-visible" className="text-xs text-[#888]">
          Visible on /offers
        </Label>
      </div>
