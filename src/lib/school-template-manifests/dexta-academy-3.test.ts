...
describe("Dexta Academy 3 manifest", () => {
  it("extracts the full template content and image assets from app public HTML", () => {
    ...
    assert.match(
      ...
      /ChatGPT_Image_Apr_22_2026_03_04_47_PM_szdmih\.png/,
	    ...
	    assert.match(String(programmes?.repeatable?.items[0]?.programmeImage), /ChatGPT_Image/);
	    assert.match(String(howToApply?.fields.primaryCtaText), /Start Application/);
  ...
  it("renders previews from the complete public source files", async () => {
    ...
    assert.equal(
      ...
      "src/app/(public)/dexta-academy-3",
    );
    ...
    assert.doesNotMatch(html, /data-site-navbar/);
	    ...
	    assert.match(script, /document\.querySelectorAll\("\[data-site-navbar\]"\)/);
    ...
    assertPreviewHtml(html);
	    ...
	    assert.match(html, /<a href="\/start" aria-current="page" style="color:#f5b82e!important;font-family:&quot;Montserrat&quot;,&quot;Segoe UI&quot;,sans-serif!important;">Start<\/a>/);
	    assert.match(html, /<a href="\/story" style="color:#00ffcc!important;font-family:&quot;Montserrat&quot;,&quot;Segoe UI&quot;,sans-serif!important;">Who We Are<\/a>/);
	    assert.match(html, /<a href="\/learning" style="color:#00ffcc!important;font-family:&quot;Montserrat&quot;,&quot;Segoe UI&quot;,sans-serif!important;">Learning<\/a>/);
	    assert.match(html, /<a href="\/moments" style="color:#00ffcc!important;font-family:&quot;Montserrat&quot;,&quot;Segoe UI&quot;,sans-serif!important;">Moments<\/a>/);
	    assert.match(html, /<a href="\/apply" style="color:#00ffcc!important;font-family:&quot;Montserrat&quot;,&quot;Segoe UI&quot;,sans-serif!important;">Admissions<\/a>/);
	    assert.match(html, /<a href="\/contact-us" style="color:#00ffcc!important;font-family:&quot;Montserrat&quot;,&quot;Segoe UI&quot;,sans-serif!important;">Talk To Us<\/a>/);
			    assert.match(
			      html,
    ...
    assert.match(html, /data-dexta-template3-preview-theme="true"/);
	    assert.match(
	      html,
	      /background:rgba\(254, 0, 102, 0\.51\)!important/,
	    );
    assert.match(
      html,
    ...
    assert.match(
      html,
	      /\.site-header \.site-nav a\{color:#00ffcc!important/,
	    );
    assert.match(
      html,
    ...
    assert.match(html, /if \(primaryDefaults\.indexOf\(defaultToken\) >= 0\) return "var\(--navy\)"/);
    assert.match(html, /if \(secondaryDefaults\.indexOf\(defaultToken\) >= 0\) return "var\(--gold\)"/);
    assert.match(html, /if \(tertiaryDefaults\.indexOf\(defaultToken\) >= 0\) return "var\(--red\)"/);
    assert.match(html, /border:1px solid #fe0066!important/);
	    assert.match(
	      html,
	      /background:rgba\(254, 0, 102, 0\)!important/,
	    );
	    assert.match(html, /color:#fff7df!important;border:3px solid #fff7df!important/);
	    ...
	    assert.match(html, /--dexta-academy-3-shared-header-portal-button-bg-color/);
	    assert.match(
	      html,
    ...
    assert.match(
	  ...
	  it("renders admin-updated academy 3 how to apply section controls", async () => {
	    ...
	    assert.match(
	      html,
	      /--dexta-academy-3-home-how-to-apply-note-bullet-color/,
	    );
  ...
  it("loads google fonts for academy 3 theme and section font overrides", async () => {
    ...
    assert.match(
      html,
      /getGoogleFontStylesheetUrl\(section\.fields\[key\]\)/,
    );
  ...
  it("moves legacy academy 3 footer phone and email links into footer contact fields", () => {
    ...
    assert.match(
	  ...
	  it("renders admin-updated academy 3 hero headline colors without colliding lines", async () => {
	    ...
	    assert.match(
	      html,
	      /--dexta-academy-3-home-hero-headline-text-color/,
	    );
	    assert.match(
	      html,
	      /--dexta-academy-3-home-hero-joyful-accent-color/,
	    );
	    ...
	    assert.match(
	      styles,
	      /--dexta-academy-3-home-hero-bg-layer: color-mix\(/,
	    );
	    assert.match(
	      styles,
  ...
  it("prefers the academy 3 manifest when a stale snapshot points elsewhere", async () => {
    ...
    assert.match(
      html,
      /DXT Academy \| Joyful Learning for Bold Young Minds/,
    );
